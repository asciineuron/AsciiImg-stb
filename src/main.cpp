...
long int strided_overlap(unsigned char* image, int swath_width, int swath_height, int stride, int offset_start, unsigned char* letter) {
...
void strided_copy(unsigned char* output, int swath_width, int swath_height, int stride, int offset_start, unsigned char* letter) {
    int letter_idx = 0;
    for (int row = 0; row < swath_height; row++) {
        for (int col = 0; col < swath_width; col++) {
...
int main(int argc, char* argv[])
{
    ...
    unsigned char* all_chars_in[size_t(('z' - 'a' + 1) + ('Z' - 'A' + 1) + ('9' - '0' + 1))];
    unsigned char* all_chars_res[size_t(('z' - 'a' + 1) + ('Z' - 'A' + 1) + ('9' - '0' + 1))];
    char current_char = 'a';
    ...
    for (int i = 0; i < tot_chars; i++) {
        // TODO note just to prefill sizes, take smallest or largest and do again with stbtt_MakeCodepointBitmap
    //for (int i = 0; i < 1; i++) {
        ...
        //    (int)current_char, &char_width, &char_height, 0, 0);
         all_chars_in[i] = stbtt_GetCodepointBitmap(&font, scale, scale,
             current_char, &char_widths[i], &char_heights[i], 0, 0);
    ...
    //for (int i = 0; i < 1; i++) {
        //all_chars[i] = stbtt_GetCodepointBitmap(&font, 0, stbtt_ScaleForMappingEmToPixels(&font, text_pixel_height),
        //    (int)current_char, &char_width, &char_height, 0, 0);
        all_chars_res[i] = (unsigned char*)malloc(char_width*char_height*sizeof(char));
        stbtt_MakeCodepointBitmap(&font, all_chars_res[i], char_width, char_height, char_width, 
   ...
   // stbi_write_jpg("./testoutscuba.jpg", ii_x, ii_y, 1, input_image, 80);
    ...
    unsigned char* output_image = (unsigned char*) malloc(output_x*output_y*sizeof(char));
    memset(output_image, 0,  (ascii_per_row*ascii_per_col)*(char_width*char_height));
    ...
    for (int i = 0; i < ascii_per_col; i++) {
        for (int j = 0; j < ascii_per_row; j++) {
            ...
            for (int char_idx = 0; char_idx < tot_chars; char_idx++) {
                long overlap = strided_overlap(input_image, char_width, char_height, ii_x, offset_start, all_chars_res[char_idx]);
